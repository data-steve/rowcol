...
### **🚨 CRITICAL: Implement Real QBO Collections API** *Effort: 4h* - **P0 CRITICAL FIX**
- **Problem**: `domains/ar/services/collections.py` contains mocked reminder functionality that doesn't actually send emails or integrate with QBO collections API.
- **Impact**: Collections functionality is non-functional and misleading to users.
- **Solution**: Implement real QBO collections API integration for basic reminder functionality.
- **Tasks**:
  - **[ ] Implement real QBO collections API** in `CollectionsService.send_reminder()` *Effort: 3h*
    - Replace mock implementation with real QBO API calls
    - Use QBO's built-in collections/reminder functionality
    - Add proper error handling for QBO API responses
  - **[ ] Add basic collections tracking** *Effort: 1h*
    - Track reminder attempts in database
    - Add collection activity logging
    - Update tray service to use proper domain service (already done)
