...
from runway.services.calculators.impact_calculator import ImpactCalculator
from runway.services.data_orchestrators.reserve_runway import RunwayReserveService
from infra.config.exceptions import TrayItemNotFoundError
...
class TrayService:
    def __init__(self, db: Session, business_id: str):
        ...
        self.runway_calculator = RunwayCalculator(db, business_id, validate_business=False) if business_id else None
        self.priority_calculator = PriorityCalculator(db, business_id, validate_business=False) if business_id else None
        
        # Stateless impact calculators (no dependencies)
        self.impact_calculator = ImpactCalculator(db, business_id, validate_business=False) if business_id else None
        ...
        try:
            ...
            if "bills" in tray_data:
                for bill in tray_data["bills"]:
                    ...
                    # Calculate impact using consolidated calculator
                    impact_data = {}
                    if self.impact_calculator and runway_context:
                        impact_data = self.impact_calculator.calculate_bill_impact(bill_data, runway_context)
            ...
            if "invoices" in tray_data:
                for invoice in tray_data["invoices"]:
                    ...
                    if runway_context:
                        ...
                        impact_data = self.impact_calculator.calculate_tray_item_impact(item_data, runway_context)
    ...
    def _calculate_runway_impact(self, item: TrayItem) -> Dict[str, Any]:
        ...
        # Calculate impact using stateless calculator
        return self.impact_calculator.calculate_tray_item_impact(item_data, runway_context)
    ...
    async def _process_bill_payment(self, item: TrayItem, confirmation_data: Dict[str, Any]) -> Dict[str, Any]:
        """Process bill payment - QBO-honest data querying only."""
        try:
            from domains.ap.services.bill import BillService
            
            # Use QBO-honest BillService for data querying only
            ...
            # QBO-honest: Only provide bill data, no execution
            return {
                "success": True,
                "message": f"Bill data retrieved for {item.title} - execution requires Ramp integration",
                "qbo_bill_id": bill.qbo_bill_id,
                "amount": float(bill.amount),
                "status": "data_only",
                "note": "Payment execution moved to _parked/ap_execution/"
            }
    ...
    def _schedule_payment(self, item: TrayItem, confirmation_data: Dict[str, Any]) -> Dict[str, Any]:
        """Schedule payment - QBO-honest data querying only."""
        try:
            from domains.ap.services.bill import BillService
            from datetime import datetime
            ...
            return {
                "success": True,
                "message": f"Bill data retrieved for {item.title} - scheduling requires Ramp integration",
                "scheduled_date": scheduled_date.isoformat(),
                "qbo_bill_id": bill.qbo_bill_id,
                "amount": float(bill.amount),
                "status": "data_only",
                "note": "Payment scheduling moved to _parked/ap_execution/"
            }
