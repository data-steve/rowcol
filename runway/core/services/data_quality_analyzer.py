...
class DataQualityAnalyzer(TenantAwareService):
    ...
    def _assess_health_level(self, hygiene_score: int) -> Dict[str, str]:
        """Assess health level based on hygiene score using industry-standard thresholds."""
        # Updated thresholds based on multi-dimensional scoring system
        if hygiene_score >= 90:
            return {
                "level": "excellent",
                "message": "Your QBO data is in excellent shape for accurate runway tracking!"
            }
        elif hygiene_score >= 80:
            return {
                "level": "good",
                "message": "Your QBO data is mostly clean with room for improvement."
            }
        elif hygiene_score >= 70:
            return {
                "level": "fair",
                "message": "Some data quality issues are affecting your runway accuracy."
            }
        elif hygiene_score >= 60:
            return {
                "level": "needs_attention",
                "message": "Several data quality issues need attention for accurate runway tracking."
            }
        elif hygiene_score >= 50:
            return {
                "level": "poor",
                "message": "Your data quality needs improvement for reliable runway analysis."
            }
        else:
            return {
                "level": "critical",
                "message": "Your data quality needs immediate attention for reliable runway analysis."
            }
