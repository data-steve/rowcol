...
Core service for calculating runway metrics and cash flow projections.
This is fundamental business logic that multiple parts of the system use:

- Weekly Digest: Current runway calculations
- Test Drive: Historical runway analysis  
- Console: Real-time runway monitoring
- Analytics: Runway trend analysis

Key Calculations:
- Current runway days based on cash position and burn rate
- Projected runway with AP/AR optimization
- Runway impact of specific financial decisions
...
class RunwayCalculator(TenantAwareService):
    """Core service for runway calculations and cash flow analysis."""
    ...
    def calculate_bill_runway_impact(self, bill_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get current runway data
            current_runway_data = self.calculate_current_runway({})
            current_runway = current_runway_data.get('base_runway_days', 0)
            daily_burn = current_runway_data.get('burn_rate', {}).get('daily_burn', 1)
    ...
    def calculate_tray_item_runway_impact(self, item_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            item_data: {"amount": float, "type": str}
        
        Returns:
            Dictionary with cash_impact and days_impact
        """
        try:
            ...
            # Calculate impact (positive for income items like invoices, negative for expenses like bills)
            if 'invoice' in item_type or 'collection' in item_type:
                # Invoices add to runway
                days_impact = amount / daily_burn if daily_burn > 0 else 0
                impact_type = 'positive'
            else:
                # Bills and other items reduce runway
                days_impact = -(amount / daily_burn) if daily_burn > 0 else 0
