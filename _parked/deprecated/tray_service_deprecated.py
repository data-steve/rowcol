...
class TrayService:
    ...
    def _calculate_runway_impact(self, item: TrayItem) -> Dict[str, Any]:
        """Calculate the runway impact of resolving this tray item using canonical TrayPriorityCalculator."""
    ...
    def categorize_bill_urgency(self, bill_data: Dict[str, Any]) -> str:
        """
        Categorize a bill as 'must_pay' or 'can_delay' using canonical PaymentPriorityCalculator.
        ...
        try:
            ...
            for item in base_items:
                ...
                if item.get("type") in ["overdue_bill", "upcoming_bill", "bill_approval"]:
                    ...
                    # Use canonical TrayPriorityCalculator for comprehensive analysis
                    if self.tray_priority_calculator:
                        priority_analysis = self.tray_priority_calculator.calculate_tray_item_priority(analysis_data)
                        
                        # Enhance the tray item with canonical analysis
                        enhanced_item.update({
                            "payment_category": priority_analysis.get("payment_category", "must_pay"),
                            "runway_impact": priority_analysis.get("runway_impact", {}),
                            "priority_score": priority_analysis.get("priority_score", 50.0),
                            "priority_level": priority_analysis.get("priority_level", "medium"),
                            "urgency_factors": priority_analysis.get("urgency_factors", []),
                            "payment_recommendation": priority_analysis.get("payment_recommendation", "")
                        })
                    else:
                        # Fallback if canonical services not available
                        enhanced_item.update({
                            "payment_category": "must_pay",
                            "runway_impact": {},
                            "priority_score": 50.0,
                    ...
                    if enhanced_item.get("payment_category") == "must_pay":
                        enhanced_item["priority"] = "high"
                        enhanced_item["urgency_level"] = "critical" if enhanced_item.get("priority_level") == "urgent" else "high"
                    else:
    ...
    def create_tray_item(self, business_id: str, item_type: str, title: str, amount: float, due_date: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        Args:
            ...
            item_type: Type of tray item (e.g., 'bill_payment')
            title: Title of the tray item
            amount: Amount associated with the item
            due_date: Optional due date for the item
            metadata: Optional metadata for additional information
        
        Returns:
            Dict representing the tray item
        """
        try:
            tray_item = {
