...
def test_create_transaction(db, test_business):
    """
    ...
    transaction = service.create_transaction(transaction_data, test_business.business_id)
    
    assert transaction.amount == 150.0
    assert transaction.firm_id == test_business.business_id
    assert transaction.description == "Starbucks Purchase"

def test_list_transactions(mock_qbo, db: Session, test_business):
    """
    Test listing bank transactions with tenant isolation.
    """
    service = BankTransactionService(db)
    transaction_data = BankTransactionCreate(
        amount=150.0,
    ...
    service.create_transaction(transaction_data, test_business.business_id)
    
    transactions = service.list_transactions(test_business.business_id)
    assert len(transactions) == 1
    assert transactions[0].amount == 150.0
    assert transactions[0].firm_id == test_business.business_id

def test_categorize_transaction(mock_qbo, db: Session, test_business):
    """
    Test categorizing an existing transaction with tenant isolation.
    """
    service = BankTransactionService(db)
    transaction_data = BankTransactionCreate(
        firm_id=test_business.business_id,
        amount=150.0,
        date=datetime.utcnow(),
        description="Starbucks Purchase",
        source="qbo_feed"
    ...
    transaction = service.create_transaction(transaction_data, test_business.business_id, test_client.client_id)
    ...
    updated_transaction = service.categorize_transaction(test_business.business_id, categorize_data)
...
def test_create_transaction_api(client: TestClient, db: Session, test_business, mock_qbo):
    """
    ...
    response = client.post(
        ...
        params={"firm_id": test_business.business_id, "client_id": test_client.client_id}
    )
    
    assert response.status_code == 200
    data = response.json()
    assert data["firm_id"] == test_business.business_id
    assert data["client_id"] == test_client.client_id
    assert data["amount"] == 150.0

def test_list_transactions_api(client: TestClient, db: Session, test_business, mock_qbo):
    """
    ...
    service.create_transaction(transaction_data, test_business.business_id, test_client.client_id)
    ...
    data = response.json()
    assert len(data) == 1
    assert data[0]["firm_id"] == test_business.business_id
    assert data[0]["client_id"] == test_client.client_id

def test_categorize_transaction_api(client: TestClient, db: Session, test_business, mock_qbo):
    """
