...
# from domains.qbo.auth import QBOAuthService, QBOEnvironment  # Lazy import to avoid circular dependency
...
class QBOAPIClient(BatchAPIClient):
    """
    Production QBO API client that makes real API calls.
    ...
    def __init__(self, business_id: str, realm_id: str, db: Session):
        ...
        # Initialize base client with QBO-specific configuration
        super().__init__(
            platform=APIPlatform.QBO,
            business_id=business_id,
            rate_limit_config=create_rate_limit_config(APIPlatform.QBO),
            retry_config=create_retry_config(APIPlatform.QBO)
        )
    ...
    def get_auth_headers(self) -> Dict[str, str]:
        ...
        auth_service = QBOAuthService(
            self.db, 
            self.business_id, 
            override_environment=QBOEnvironment.SANDBOX if "sandbox" in self.get_base_url() else None
        )
        
        access_token = auth_service.get_valid_access_token()
        if not access_token:
            raise AuthenticationError(f"No valid QBO access token for business {self.business_id}")
    ...
    def handle_error_response(self, response: httpx.Response) -> APIError:
        """Handle QBO API error responses."""
        if response.status_code == 401:
            return AuthenticationError(
                "QBO authentication failed - token may be expired",
                status_code=401
            )
            ...
            return APIError(
                f"QBO server error: {response.status_code}",
    ...
    async def make_batch_request(self, batch_operations: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Make a batch API request to QBO."""
        return await self._make_batch_api_call(batch_operations)
    ...
    async def _make_batch_api_call(self, batch_queries: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        from domains.qbo.auth import QBOAuthService  # Lazy import to avoid circular dependency
