...
from domains.integrations.qbo.qbo_connection_manager import (
    QBOConnectionManager,
    QBOConnectionStatus,
    QBOConnectionHealth,
    get_qbo_connection_manager
)
...
class TestSmartSyncServiceIntegration:
    ...
    def test_smart_sync_initialization(self, mock_db):
        """Test SmartSyncService initializes with QBO connection manager."""
        smart_sync = SmartSyncService(mock_db, TEST_BUSINESS_ID)
        
        assert smart_sync.qbo_connection_manager is not None
        assert smart_sync.business_id == TEST_BUSINESS_ID
    
    @patch('asyncio.new_event_loop')
    @patch('asyncio.set_event_loop')
    def test_get_qbo_data_for_digest_healthy_connection(self, mock_set_loop, mock_new_loop, mock_db):
        """Test digest data retrieval with healthy QBO connection."""
        # Mock event loop
        mock_loop = Mock()
        mock_new_loop.return_value = mock_loop
        
        # Mock successful connection and API calls
        mock_loop.run_until_complete.side_effect = [
            True,  # ensure_healthy_connection
            {"QueryResponse": {"Bill": []}},  # bills query
            {"QueryResponse": {"Invoice": []}},  # invoices query
            {"QueryResponse": {"Account": []}}   # accounts query
        ]
        
        smart_sync = SmartSyncService(mock_db, TEST_BUSINESS_ID)
        result = smart_sync.get_qbo_data_for_digest()
        
        assert result["source"] == "qbo_api"
